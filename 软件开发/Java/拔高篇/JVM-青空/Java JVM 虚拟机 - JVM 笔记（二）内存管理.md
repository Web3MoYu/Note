...
- 新生代回收（将新生代回收，将存活的放到survivor区，然后将survivor区中活的时间长的放到老年代）
- 并发标记（当老年代超过阈值默认45%，会标记老年代中存活的对象）
- 混合收集（将老年代中回收价值高的和新生代，survivor区的一块进行收集）可能需要多次
- 并发失败(回收速度赶不上创建速度)会触发FULL GC
